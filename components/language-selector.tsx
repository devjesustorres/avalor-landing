  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        ...
        aria-expanded={open}
        className="flex items-center gap-1.5 bg-slate-200/60 dark:bg-slate-800/60 neu-badge pl-2.5 pr-2 py-1.5 rounded-full text-xs font-bold text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white transition-colors"
      >
      ...
      {open && (
        <ul
          role="listbox"
          className="absolute right-0 top-full mt-2 min-w-[160px] rounded-xl bg-white dark:bg-slate-900 shadow-xl shadow-black/10 dark:shadow-black/50 border border-slate-200/70 dark:border-slate-800 p-1.5 z-50"
        >
          {options.map((o) => {
